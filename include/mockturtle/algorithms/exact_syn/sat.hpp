...
using mig_node = std::pair<uint64_t, std::vector<size_t>>;
...
std::map<size_t, std::vector<uint64_t>> primitives = {
    { 3u, primitives_3 },
    { 5u, primitives_5 },
};

std::vector<uint64_t> get_primitives( size_t fanin )
{
...
public:
  sat_enc( const aqfp_logical_network& _net, size_t _num_in, uint64_t _tt, size_t _verbose = 0 )
  {
    net = _net;
    num_in = _num_in;
    tt = _tt;
  ...
  std::string chain_as_string()
  {
    ...
    std::map<size_t, uint64_t> node_func_map;
    ...
    for ( auto n = 0u; n < num_nodes; n++ )
    {
      ...
      {
        if ( (size_t)net.nodes[n][i] >= num_nodes )
        {
